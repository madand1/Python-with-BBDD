    ...
    try:
        cursor.execute("""
            SELECT COUNT(*) AS total_profesores, 
                   nombreprofesor || ' ' || apellido1 || ' ' || apellido2 AS nombre_completo
            FROM profesor
...
def obtener_asignaturas_por_profesor(cursor):
    ...
    try:
        cursor.execute("""
            SELECT 
                COALESCE(p.nombreprofesor || ' ' || p.apellido1 || ' ' || p.apellido2, 'Profesor Sustituto') AS nombre_completo,
                a.nombre AS asignatura
            FROM asignatura a
            LEFT JOIN contrato c ON a.id_asignatura = c.id_asignatura
    ...
    try:
        ...
        if connection and cursor:
            while True:
                ...
                cursor.execute("SELECT id_alumno FROM alumno WHERE dni = :dni", dni=dni)
                resultado = cursor.fetchone()
                if resultado:
                    nueva_nota = float(input("Ingrese la nueva nota: "))  # Cambio aquí para permitir decimales
                    cursor.execute("UPDATE notas SET calificacion = :nueva_nota WHERE id_alumno = :id_alumno", nueva_nota=nueva_nota, id_alumno=resultado[0])
...
def insertar_alumno(cursor):
    ...
    try:
        ...
        # Insertar el alumno
        cursor.execute("INSERT INTO alumno (id_alumno, nombre, apellido1, apellido2, dni, id_grupo, codigo_postal, id_direccion) VALUES (:id_alumno, :nombre, :apellido1, :apellido2, :dni, :id_grupo, :codigo_postal, :id_direccion)", id_alumno=id_alumno, nombre=nombre, apellido1=apellido1, apellido2=apellido2, dni=dni, id_grupo=id_grupo, codigo_postal=codigo_postal, id_direccion=id_direccion)
...
def eliminar_alumno_por_dni(cursor):
    ...
    try:
        for intento in range(3):
            ...
            cursor.execute("SELECT COUNT(*) FROM alumno WHERE dni = :dni", dni=dni)
            cantidad_alumnos = cursor.fetchone()[0]
            if cantidad_alumnos == 0:
                print("No se encontró ningún alumno con el DNI proporcionado.")
            else:
                cursor.execute("DELETE FROM alumno WHERE dni = :dni", dni=dni)
                cursor.connection.commit()
...
def mostrar_menu():
    ...
    print("6. Incorporacion de nuevo alumno.")
